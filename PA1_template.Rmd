...
```{r, echo=TRUE}
numNA <- sqldf("select count(*) NUM 
                from acts
                where steps is NULL")
numNA
NAs <- sqldf("select a.date, a.interval, b.avgstep
                from acts a, stepxday b
                where a.steps is NULL
                and a.interval = b.interval) 
ALLsteps <- sqldf("select date, interval, steps 
                     from acts a
                   ...
                   select *
                     from NAs
                  order by 1,2")
stepdataB <- sqldf("select date, avg(steps) as avgstep, sum(steps) as sumsteps
                      from ALLsteps
                      group by date")
...
Weekday analysis
```{r, echo=TRUE}
weekdata <- sqldf("select *,dayofweek(date) dow from ALLsteps")
